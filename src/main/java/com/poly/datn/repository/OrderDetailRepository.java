...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, Integer> {
    ...
    @Query(value = "SELECT SUM(order_detail.price_sum - order_detail.promotion_value*order_detail.quantity) " +
            ...
            "          WHERE order_detail.create_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 1 MONTH) AND LAST_DAY(DATE_SUB(CURDATE(), INTERVAL 1 MONTH)) and DAYOFWEEK(order_detail.create_date) = :day " +
    ...
    @Query(value = "SELECT SUM(order_detail.price_sum - order_detail.promotion_value*order_detail.quantity) " +
            "            FROM order_detail " +
            "            inner join orders  on orders.id = order_detail.order_id " +
            "            WHERE MONTH(order_detail.create_date) = MONTH(CURDATE()) AND YEAR(order_detail.create_date) = YEAR(CURDATE()) and DAYOFWEEK(order_detail.create_date) = :day " +
            "            GROUP BY DAYOFWEEK(order_detail.create_date)", nativeQuery = true)
